...
### שלב 3 — שאל 4 שאלות בבת אחת

**אל תשאל אחת-אחת.** הצג את כולן יחד, תן למשתמש לענות על מה שרלוונטי:

```
כדי שנתאים את הסורק אליך, אשמך ל-4 דברים (כולם אופציונליים — אפשר לדלג):

1. 🎯 קריטריונים: איפה מחפשים? תקציב? חדרים? מועד כניסה?
   (ברירת מחדל: רמת גן/גבעתיים, עד 6,500₪, 3 חדרים, סוף יולי 2026)

2. 🤖 טלגרם: יש לך בוט? (טוקן מ-@BotFather)
   (בלעדיו — הדוחות יישמרו פה במחשב)

3. 🍪 פייסבוק: יש לך קובץ עוגיות מפייסבוק?
   (בלעדיו — ייסרקו רק קבוצות ציבוריות)

4. 🧠 Gemini API: יש לך מפתח?
   (בלעדיו — סינון בסיסי בלבד, בלי AI)
```
