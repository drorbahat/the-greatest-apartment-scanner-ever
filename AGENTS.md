...
**⚠️ חובה להציג את הטקסט הבא מילה במילה (או קרוב מאוד). אל תסכם, אל תקצר.**

**הצג את כל הארבעה יחד.** אם המשתמש עונה על הכל — תן לו לאשר, ואז המשך לשלב 3.5.

---

#### 1. 🎯 קריטריוני חיפוש

**הצג למשתמש (טקסט מדויק):**

```
🎯 קריטריונים: איפה מחפשים? תקציב? חדרים? כניסה?

ברירת מחדל: רמת גן/גבעתיים, עד ₪6,500, 3 חדרים, סוף יולי 2026.
אם אתה מחפש משהו אחר — פשוט תגיד.
```
...
```
🍪 פייסבוק — מה זה נותן?
  • גישה לקבוצות סגורות — איפה שרוב הדירות מתפרסמות
  • הסורק נכנס ל-12+ קבוצות, גולל, מחלץ, מנקה, מסנן
  • בלעדיו: רק Yad2 (טוב, אבל מפספס המון)

⚠️  צריך חשבון פייסבוק נפרד — לא החשבון האישי!
  (הסורק מדפדף אוטומטית — פייסבוק עלולה לחסום חשבון)

מה צריך:
  1. צור חשבון פייסבוק חדש (שם אנונימי — "מחפש דירה")
  2. הצטרף ל-10–15 קבוצות דירות באזור שלך
  3. התקן תוסף: Get cookies.txt (Chrome) או Export Cookies (Firefox)
  4. התחבר לחשבון בדפדפן ← ייצא עוגיות ← שמור כ-cookies.json
  5. תן לי את הקובץ — אני אשים ב-data/

  (10 דקות. רוצה שאסביר יותר? או שנדלג?)
```
...
```
🧠 Gemini API — מה זה נותן?
  • AI קורא פוסטים, מבין תוכן, מחלץ מחיר/חדרים/כניסה בחכמה
  • הפיכת "דירה מהממת 3 חדרים באיזור המבוקש" ← מחיר: ₪6,200, חדרים: 3
  • בלעדיו: סינון לפי מילות מפתח — עובד, אבל פחות מדויק

💰 חינם! (עד 1,500 בקשות ביום)

איך משיגים (3 דקות):
  1. כנס ל-aistudio.google.com/apikey
  2. לחץ "Create API Key"
  3. העתק את המפתח — תן לי אותו, אני אשים ב-.env

  (3 דקות, חינם. רוצה שאסביר יותר? או שנדלג?)
```
...
```
🤖 טלגרם — מה זה נותן?
  • דוחות דירות נשלחים ישר לטלפון — בלי מחשב
  • פקודות: /scan, /report, /recent
  • התראות אוטומטיות כל שעה
  • בלעדיו: דוחות נשארים קבצים בתיקיית artifacts/

איך משיגים (2 דקות):
  1. פתח את @BotFather בטלגרם
  2. שלח /newbot ← תן שם ← תן username
  3. BotFather שולח Token — תן לי אותו

  (2 דקות. רוצה שאסביר יותר? או שנדלג?)
```
...
- ❌ **אסור להשתמש בטבלאות.** טבלאות בעברית נראות זוועה. השתמש תמיד ברשימות ונקודות.
...
- ❌ אל תערוך את Makefile — אין סיבה. הוא עובד כמו שהוא.
